...
def extract_features(file_path, duration=30):
    ...
    try:
        ...
        # onset_strength gives us a graph of energy spikes over time
        ...
        if len(beat_frames) > 1:
            ...
            features.update(_summarize(beat_intervals, "beat_interval"))
        else:
        ...
        for i in range(mfcc.shape[0]):
            features.update(_summarize(mfcc[i], f"mfcc_{i+1}"))
            features.update(_summarize(mfcc_delta[i], f"mfcc_delta_{i+1}"))
            features.update(_summarize(mfcc_delta_2[i], f"mfcc_delta_2_{i+1}"))
        ...
        features.update(_summarize(spectral_centroid, "spectral_centroid"))
        features.update(_summarize(spectral_bandwidth, "spectral_bandwidth"))
        features.update(_summarize(spectral_rolloff, "spectral_rolloff"))
        features.update(_summarize(spectral_flatness, "spectral_flatness"))

        for i in range(spectral_contrast.shape[0]):
            features.update(
                _summarize(spectral_contrast[i], f"spectral_contrast_{i+1}")
            )

        # harmonics and pitch
        chroma = librosa.feature.chroma_stft(y=raw_waveform, sr=sample_rate)
        for i in range(chroma.shape[0]):
            features.update(_summarize(chroma[i], f"chroma_{i+1}"))
        ...
        for i in range(tonnetz.shape[0]):
            features.update(_summarize(tonnetz[i], f"tonnetz_{i+1}"))
        ...
        features.update(_summarize(root_mean_square_value, "rms"))
        features.update(_summarize(zero_crossing_rate, "zcr"))
